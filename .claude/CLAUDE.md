...
- `closing_odds` and `clv` are populated automatically by cron jobs — never set manually
- `result` is populated by the results cron, but can also be set manually from the dashboard (ResultEditor in BetTable) for bets with no API coverage; manual settling recomputes `profit_loss` with the same formula as the cron
